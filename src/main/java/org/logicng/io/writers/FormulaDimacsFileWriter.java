...
public final class FormulaDimacsFileWriter {
  ...
  public static void write(final String fileName, Formula formula, boolean writeMapping) throws IOException {
    ...
    if (!formula.holds(new CNFPredicate())) {
      CNFEncoder encoder = new CNFEncoder(formula.factory(), new CNFConfig.Builder().algorithm(CNFConfig.Algorithm.FACTORIZATION).build()); //TODO exact config?
      formula = encoder.encode(formula);
    }
    ...
    sb.append(formula.variables().size()).append(" ").append(parts.size()).append("\n");
